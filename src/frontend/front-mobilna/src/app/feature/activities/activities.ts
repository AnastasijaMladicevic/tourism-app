      ...
      for (let start = 2; start <= totalPages; start += batchSize) {
        ...
        const batch = await Promise.all(
          Array.from({ length: end - start + 1 }, (_, i) =>
            firstValueFrom(
              this.activityService.getPage({ page: start + i, pageSize, sortBy: 'name', sortOrder: 'asc' })
            ).catch(() => ({ items: [] as ActivityDto[], totalPages, page: start + i, pageSize, totalCount: 0 }))
          )
        );
        allItems = [...allItems, ...batch.flatMap(p => p.items ?? [])];
        if (currentToken === this.loadToken) {
          this.applyActivitiesData(allItems);
        }
      }
