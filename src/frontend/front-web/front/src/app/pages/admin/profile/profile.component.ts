...
export class ProfileComponent implements OnInit, OnDestroy {
  ...
  get hasCustomProfileImage(): boolean {
    const currentImage = this.avatarUrl?.trim();
  
    if (!currentImage || currentImage === ProfileComponent.DEFAULT_PROFILE_IMAGE_URL) {
      return false;
    }
  
    return !currentImage.toLowerCase().includes('/images/profiles/default_icon.png');
  }
  ...
  private syncUserState(user: UserDto): void {
    this.user = {
      ...
      country: ProfileComponent.normalizeCountry(user.country),
