...
namespace TuristickiVodic.Tests.Controllers
{
    public class EventsControllerTests
    {
        ...
        [Fact]
        public async Task GetById_KadaJeManagerNadlezan_VracaNeodobrenEventIzSvojeDestinacije()
        {
            var mockService = new Mock<IEventService>();
            var dto = new EventDto { Id = 5, Name = "Pending event za odobravanje", Status = "Pending" };
        ...
        public async Task GetForManager_Manager_VracaPagedRezultat()
        {
            ...
            var dto = new PagedResultDto<EventDto>
            {
                Items = new List<EventDto>
                {
                    new EventDto { Id = 1, Name = "Pending event", Status = "Pending" }
                },
                Page = 1,
                PageSize = 10,
                TotalCount = 1,
            ...
            mockService
                .Setup(s => s.GetForManagerAsync(10, It.IsAny<EventQueryDto>()))
                .ReturnsAsync(dto);

            var controller = CreateController(mockService, FakeUserHelper.CreateUser(10, "Manager"));
            var result = await controller.GetForManager(new EventQueryDto());
