...
namespace TuristickiVodic.Tests.Controllers
{
    public class ReviewsControllerTests
    {
        ...
        {
            var mockService = new Mock<IReviewService>();
            var dto = new ReviewDto { Id = 10, Text = "Super", Status = "Approved" };
            mockService.Setup(s => s.CreateAsync(It.IsAny<CreateReviewDto>(), 5, "Tourist")).ReturnsAsync(dto);

            var controller = CreateController(mockService, FakeUserHelper.CreateUser(5, "Tourist"));
            var result = await controller.Create(new CreateReviewDto { ObjectId = 1, Rating = 5, Text = "Super" });
        ...
        public async Task Create_KadaServisBaciInvalidOperation_VracaBadRequest()
        {
            ...
            var controller = CreateController(mockService, FakeUserHelper.CreateUser(5, "Tourist"));
            var result = await controller.Create(new CreateReviewDto { ObjectId = 1, Rating = 5, Text = "Test" });
        ...
        {
            var mockService = new Mock<IReviewService>();
            var dto = new ReviewDto { Id = 1, Text = "Novi tekst", Status = "Approved" };
            mockService.Setup(s => s.UpdateAsync(1, It.IsAny<UpdateReviewDto>(), 5, "Tourist")).ReturnsAsync(dto);
        ...
        public async Task Respond_ContentCreator_VracaOk()
        {
            ...
            mockService.Setup(s => s.RespondAsync(1, It.IsAny<RespondToReviewDto>(), 20, "ContentCreator")).ReturnsAsync(dto);
        ...
        public async Task UpdateResponse_ContentCreator_VracaOk()
        {
            ...
            mockService.Setup(s => s.UpdateResponseAsync(1, It.IsAny<RespondToReviewDto>(), 20, "ContentCreator")).ReturnsAsync(dto);
        ...
        public async Task DeleteResponse_ContentCreator_VracaOk()
        {
            ...
            mockService.Setup(s => s.DeleteResponseAsync(1, 20, "ContentCreator")).ReturnsAsync(dto);
        ...
        {
            var mockService = new Mock<IReviewService>();
            mockService.Setup(s => s.DeleteAsync(1, 5, "Tourist")).ThrowsAsync(new UnauthorizedAccessException());
